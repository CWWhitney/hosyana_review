plot_sankey_clean <- function(inputs, losses, unit, labels, format = "plot") {
  ########################
  # Clean SankeyR version - modified for consistent text and no percentages
  # Based on SankeyR version 1.01 by Aaron BERDANIER
  #
  # OPTIONS:
  ...
  #
  # MODIFICATIONS:
  # - Consistent text size (fontsize = 0.8 for all labels)
  # - No percentages in labels
  # - Count only at input (leftmost)
  ########################

  # Calculate fractional losses and inputs
  # Handle multi-method papers by using total losses instead of inputs for percentage calculation
  total_losses <- sum(losses)
  frLosses <- losses / total_losses
  frInputs <- inputs / sum(inputs)

  # Modified labels - input with count, losses with percentages
  inputLabel <- paste(labels[1], ": ", inputs[1], " ", unit, sep = "")
  lossLabel <- paste(labels[length(labels)], ": ", round(100 * frLosses[length(frLosses)], 1), "%", sep = "")

  ########################
  # Calculate position of plot axes
  limTop <- frInputs[1]
  posTop <- 0.4
  maxy <- 0
  limBot <- 0
  ...
  # Calculate fractional losses and inputs
  # Handle multi-method papers by using total losses for calculations
  total_losses <- sum(losses)
  frLosses <- losses / total_losses
  frInputs <- inputs / sum(inputs)
  ...
  # First input label - CONSISTENT TEXT SIZE
  inputLabel <- paste(labels[1], ": ", inputs[1], " ", unit, sep = "")
  fontsize <- 0.8 # FIXED FONT SIZE
  text(0, frInputs[1] / 2, inputLabel, cex = fontsize, pos = 2)
  ...
  for (i in 1:(length(losses) - 1)) {
    ...
    fullLabel <- paste(labels[i + length(inputs)], ": ", round(100 * frLosses[i], 1), "%", sep = "")
    fontsize <- 0.8 # FIXED FONT SIZE
  ...
  # Last loss label - CONSISTENT TEXT SIZE, WITH PERCENTAGES
  lossLabel <- paste(labels[length(labels)], ": ", round(100 * frLosses[length(frLosses)], 1), "%", sep = "")
  fontsize <- 0.8 # FIXED FONT SIZE
  text(newPos + 0.05, limTop - frLosses[length(frLosses)] / 2, lossLabel, cex = fontsize, pos = 4)
