run_prisma_pipeline <- function(bib_data) {
  ...
  # Step 2: Duplicate identification and REMOVAL
  cat("1. Identifying and removing duplicates...\n")
  bib_data <- identify_duplicates(bib_data)
  step_counts$duplicates_removed <- sum(bib_data$duplicate_status != "unique")
  # ACTUALLY REMOVE duplicates
  bib_data <- bib_data %>% filter(duplicate_status == "unique")
  step_counts$after_duplicates <- nrow(bib_data)
  cat("   Removed", step_counts$duplicates_removed, "duplicates\n")
  cat("   Records remaining:", step_counts$after_duplicates, "\n\n")
  
  # Step 3: Manual classifications and REMOVAL
  ...
  # ACTUALLY REMOVE manually excluded records
  bib_data <- bib_data %>% filter(manual_decision != "exclude")
  ...
  cat("   Records remaining after manual exclusion:", nrow(bib_data), "\n\n")
  
  # Step 4: Automated exclusion rules and REMOVAL
  ...
  # ACTUALLY REMOVE automated exclusions (only from unreviewed records)
  bib_data <- bib_data %>% 
  ...
  cat("   Records remaining after automated exclusion:", nrow(bib_data), "\n\n")
  
  # Step 5: Final classification (just for tracking, no removal needed)
  ...
  return(list(
    classified_data = bib_data,  # This now contains ONLY the records that passed all filters
