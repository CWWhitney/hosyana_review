generate_prisma_stats <- function(bib_data) {
  ...
  stats <- list(
    ...
    screening = list(
      ...
      manually_excluded_bin = sum(bib_data$manual_decision == "exclude"),
      not_relevant_automated = sum(bib_data$automated_exclude & bib_data$manual_decision == "unreviewed" & bib_data$exclusion_reason == "irrelevant_content"),
      not_fitting_assessment = sum(bib_data$automated_exclude & bib_data$manual_decision == "unreviewed" & bib_data$exclusion_reason == "invalid_document_type"),
    ...
    included = list(
      ...
      manually_included = sum(bib_data$manual_decision == "include"),
      automated_included = sum(bib_data$final_decision == "include" & bib_data$manual_decision == "unreviewed"),
      quantitative_assessment = sum(bib_data$final_decision == "include"),
      qualitative_synthesis = sum(bib_data$manual_decision == "include"),  # Only manually reviewed for qualitative
